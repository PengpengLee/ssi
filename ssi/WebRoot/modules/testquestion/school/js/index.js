...
function getById(id) {
	...
	$.ajax({
		url: ctx + '/school/getSchoolById.action',
		type: 'GET',
		data: {id: id},
		dataType: 'json',
		success: function(msg) {
			if(msg.key == 'success') {
				var school = msg.obj;
				$("#add_data").find("[name='name']").val(school.name);
				$("#add_data").find("[name='id']").val(school.id);
				$("#add_data").find("[name='createTime']").val(school.createTime);
				$("#add_data").find("[name='del']").val(school.del);
...
function deleteData(id) {
	...
	Msg.confirm("确认删除？", function() {
		$.ajax({
			type : "POST",
			dataType : "json",
			url : ctx + '/school/delSchool.action',
			data : {id: _id},
			success : function(msg) {
				if(msg.key == "success") {
					/** 重新加载数据列表 */
...
$(function(){
    ...
    $('#add_data').validate({
        ...
        submitHandler: function(form) {
        	// 异步提交表单 
			$.ajax({
				url : ctx + '/school/saveSchool.action',
				type : 'POST',
				data : $(form).serialize(),
				dataType : 'json',
				success : function (msg){
